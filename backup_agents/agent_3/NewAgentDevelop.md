...
- 两阶段评分：
  ...
  - 第二阶段对 Top-4 每个候选进行 2 次带噪仿真，取“鲁棒评分”选最优（平均分 + 20×首碰己方率 − 60×白球落袋率）。
  - 局部微调：在时间预算允许时进行轻量微调（φ、V0、b 的小范围变体），其余情况下跳过以缩短思考时长。
  - 回退策略：当最佳动作平均分低于阈值（如 <10）时，评估安全球候选（2 次仿真），以“平均分 + 20×首碰己方率 − 60×白球落袋率”选取最佳，不再使用完全随机动作。
...
- 鲁棒优先：
 ...
 - 时间权衡：候选数量上限约 12、两阶段总仿真数受控（约 28/杆，安全球约 12 次以内），在保持稳健性的同时显著降低单局思考总时长。
