...
class NewAgent(Agent):
            ...
            for _ in range(sims):
                ...
                if shot is None:
                    ...
                    if 'cue' in new_pocketed:
                        cue_pocket += 1
            avg = agg / sims
            robust = avg + 20.0 * (contact / sims) - 60.0 * (cue_pocket / sims)
    ...
    def _has_clear_path(self, cue_pos, ghost_pos, balls, ignore_ids):
        ...
        for bid, ball in balls.items():
            ...
            proj = p + t * v
            if np.linalg.norm(proj - c) <= self.ball_radius * 1.1:
                return False
        return True
    ...
    def _generate_candidates(self, balls, my_targets, table):
        ...
        for tid in target_ids:
            ...
            for j, (_, pocket) in enumerate(pockets_sorted[:2]):
                ...
                if not self._has_clear_path_obj_to_pocket(obj_pos, pocket_pos, balls, ignore_ids={'cue', tid}):
                    continue
                v1 = np.array(obj_pos[:2]) - np.array(cue_pos[:2])
                v2 = np.array(pocket_pos[:2]) - np.array(obj_pos[:2])
                n1 = np.linalg.norm(v1)
                n2 = np.linalg.norm(v2)
                if n1 > 1e-6 and n2 > 1e-6:
                    cosang = float(np.clip(np.dot(v1, v2) / (n1 * n2), -1.0, 1.0))
                    cut_deg = abs(math.degrees(math.acos(cosang)))
                    if cut_deg > 75.0:
                        continue
                v_base = 1.2 + dist * 1.2
                ...
                actions.append({'V0': v_base, 'phi': (phi_ideal + 0.6) % 360, 'theta': 0.0, 'a': pos_a, 'b': -0.02})
                actions.append({'V0': v_base, 'phi': (phi_ideal - 0.6) % 360, 'theta': 0.0, 'a': -pos_a, 'b': -0.02})
                if j == 0:
                    actions.append({'V0': float(min(v_base + 0.4, 6.0)), 'phi': phi_ideal, 'theta': 0.0, 'a': pos_a, 'b': -0.02})
    ...
    def _generate_safety_candidates(self, balls, table):
        ...
        for sid in target_ids_sorted[:3]:
            sp = np.array(balls[sid].state.rvw[0][:2])
            dist_s = np.linalg.norm(sp - cp[:2])
            phi_s = self._calc_angle(sp - cp[:2])
            for dphi in [0.0, 0.6, -0.6]:
    ...
    def _local_refine(self, balls, table, action, last_state_snapshot, my_targets):
        ...
        candidates = [
            ...
            {'V0': base['V0'], 'phi': float((base['phi'] + 0.25) % 360), 'theta': base['theta'], 'a': base['a'], 'b': float(np.clip(-0.03, -0.5, 0.5))},
            {'V0': base['V0'], 'phi': float((base['phi'] - 0.25) % 360), 'theta': base['theta'], 'a': base['a'], 'b': float(np.clip(-0.03, -0.5, 0.5))},
        ]
        best = base
        best_robust = -1e9
        for act in candidates:
            ...
            sims = 2
            for _ in range(sims):
                shot = self._simulate_action(balls, table, act)
                if shot is None:
                    agg += -500.0
                else:
                    agg += self._analyze_shot_for_reward(shot, last_state_snapshot, my_targets)
    ...
    def _analyze_shot_for_reward(self, shot: pt.System, last_state: dict, player_targets: list):
        ...
        if foul_no_rail:
            score -= 30.0
        score += float(len(own_pocketed)) * 60.0
