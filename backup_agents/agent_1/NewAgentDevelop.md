...
- 两阶段评分：
  ...
  - 第二阶段对 Top-8 每个候选进行 5 次带噪仿真，取“鲁棒评分”选最优（平均分 + 20×首碰己方率 − 60×白球落袋率）。
  - 局部微调：围绕最优动作进行小范围搜索（φ±0.2°/0.4°、V0±0.2、b∈{原值, -0.03, -0.04}），每个候选进行 4 次带噪仿真，以同一鲁棒评分选择改良版本。
  - 回退策略：当最佳动作平均分低于阈值（如 <10）时，评估安全球候选（5 次仿真），以“平均分 + 20×首碰己方率 − 60×白球落袋率”选取最佳，不再使用完全随机动作。
...
- 鲁棒优先：
  - 候选限定：仅选距离白球最近的最多 4 个目标球；每球优先 3 个最近袋口。
  - φ与力度的微扰（±0.4° 与 +0.6 力度），加入小幅侧旋（|a|≈0.02）降低白球入袋风险。
- 时间权衡：候选数量上限约 24、两阶段总仿真数受控（约 84/杆），兼顾覆盖与运行时长。
