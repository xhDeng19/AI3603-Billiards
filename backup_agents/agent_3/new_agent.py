...
class NewAgent(Agent):
        ...
        for idx in top_indices:
            ...
            for _ in range(sims):
                ...
                if time.perf_counter() - start_time > budget:
                    break
            avg = agg / max(1, count)
            robust = avg + 20.0 * (contact / max(1, count)) - 60.0 * (cue_pocket / max(1, count))
    ...
    def _local_refine(self, balls, table, action, last_state_snapshot, my_targets):
        ...
        variants = []
        for dphi in [0.0, 0.2, -0.2, 0.4, -0.4]:
            for dv in [0.0, 0.2, -0.2]:
                for nb in [base['b'], -0.03, -0.04]:
                    variants.append({
                        'V0': float(np.clip(base['V0'] + dv, 0.5, 8.0)),
                        'phi': float((base['phi'] + dphi) % 360),
                        'theta': base['theta'],
                        'a': base['a'],
                        'b': float(np.clip(nb, -0.5, 0.5))
                    })
        best = base
        best_robust = -1e9
        for v in variants:
